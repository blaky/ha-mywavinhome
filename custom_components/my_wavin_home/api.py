    ...
    async def authenticate(self) -> str:
        ...
        try:
            ...
            async with session.post(
                ...
                if not self.session_id:           
                    for cookie in session.cookie_jar:
                        if cookie.key == "PHPSESSID":
                            _LOGGER.error("Reused session id from cookie jar: %s", cookie.value)
                ...
                _LOGGER.error("Session ID: %s", self.session_id)
    ...
    async def get_room_temperatures(self) -> dict[str, Any]:
        ...
        try:
            async with session.get(
                f"https://www.mywavinhome.com/thermostats",
                ...
                if response.status == 401:
                    ...
                    await self.authenticate()
                    return await self.get_room_temperatures()
                elif response.status != 200:
                    _LOGGER.error(f"API returned status: {response.status}")
                    raise ConnectionError(f"API returned status {response.status}")
                
                # Parse HTML response as XML
                    ...
                    for room in soup.select(".items .listview"):
                        ...
                        for thermData in room.select(".thermHeader2"):
                            ...
                            if text.endswith(" rh%"):
                                ...
                                temperature = text[:-2]
                        _LOGGER.info(f"Room details ({room_id}): {room_name}, {temperature}, {humidity}")

                        rooms[room_id] = {"name": room_name, "temperature": temperature, "humidity": humidity}
                    return rooms
                except Exception as e:
                    _LOGGER.error(f"Error processing HTML: {e}")
                    raise ConnectionError(f"Error processing response: {e}")
